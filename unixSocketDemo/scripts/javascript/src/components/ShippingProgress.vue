    ...
    <ul class="progress-indicator">
        <li v-for="(step, key) in steps" :key="key" :class="{ completed : step.timestamp, completed_right : (step.timestamp && key + 1 < steps.length && steps[key+1].timestamp ),  completed_left : (step.timestamp && key > 0 && steps[key-1].timestamp ) }">
            <span class="bubble" @click.prevent="completed" :id="key">
                ...
                <i v-if="step.action || transaction_in_progress_test(key)" :class="transaction_in_progress_test(key) ? transaction_in_progress_spinner : step.action" key="icon"></i>
                </transition>
            </span>
...
export default {
    ...
    data: function() {
        return {
            transaction_in_progress_spinner: 'fa fa-spinner fa-spin'
        };
    },
    methods: {
        ...
        transaction_in_progress_test: function (key) {
            return this.transaction_in_progress && !this.steps[key].timestamp && (key == 0 || this.steps[key-1] && this.steps[key-1].timestamp)
        },
        strftime: strftime
    }
}
